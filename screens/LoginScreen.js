...
export default function LoginScreen({navigation}) {
  ...
  async function checkUser(){
      const user = await db.collection('user').where('username','==',username).where('password','==',password).onSnapshot(querySnap=>{
        const users = []
      ...
      <NativeBaseProvider>
        <ScrollView>
          ...
          <Box style={[{marginTop:"10%"}]}>
            ...
            <Button mt="4" mb="3" bgColor="#B4948D" borderWidth="1" borderColor="#9D746B" shadow="7" _text={{fontSize:"md", textAlign:'center'}} onPress={()=>{checkUser()}}>Sign in</Button>
            </Box>
          </Box>
